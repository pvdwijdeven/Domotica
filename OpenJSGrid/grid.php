...
Class Grid {
	...
	function __construct($table, $options, $mysqlmeasi) {
		$this->table = $table;
		$this->mysqlmeasi = $mysqlmeasi;
	...
	function add() {
		...
		if(empty($this->set)) {
			$mysqlmeasi->query("INSERT INTO {$this->table} VALUES ()");
		
		// if you passed a set param then use that in the insert
		} else {
			$mysqlmeasi->query("INSERT INTO {$this->table} SET {$this->set}");
		}
	...
	function delete() {
		...
		$primaryKey = $this->getPrimaryKey();
		return $mysqlmeasi->query("DELETE FROM {$this->table} WHERE `$primaryKey` = '$post[id]'");
	}
		...
		if(isset($post['maxLength']) && $post['maxLength'] == "true") {
			foreach($colsArray as $col) {
				...
				if(stripos($col," as ") === false) {
					...
					$colDataSql = $mysqlmeasi->query("SHOW columns FROM $aTable WHERE Field = '$field'");
	...
	function getPrimaryKey($table=NULL) {
		...
		if ($result = mysqli_query($mysqlmeasi, "SELECT DATABASE()")) {
		$row = mysqli_fetch_row($result);
		printf("Current database is %s.\n", $row[0]);
		mysqli_free_result($result);
		}
		echo ("SHOW KEYS FROM `$table` WHERE Key_name = 'PRIMARY'\n");
		$primaryKey = mysqli_query($mysqlmeasi,"SHOW KEYS FROM `$table` WHERE Key_name = 'PRIMARY' limit 1");
		$xprimaryKey = mysql_fetch_assoc($primaryKey);
		return $xprimaryKey['Column_name'];
	}
