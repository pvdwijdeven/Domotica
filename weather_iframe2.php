...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="css/weather2.css?v=3.0">
	...
	<body>
		...
		//		}
			...
			function processCurrent(current){
				...
				$("#weather_current").html("<br>luchtvochtigheid: "+current.main.humidity+"%<BR>"+current.weather[0].description);
			...
			function processForecast(jsforecast){
					$('#weather_minmax').html("<br>max:"+jsforecast.forecast.simpleforecast.forecastday[0].high.celsius+"&deg<BR>min:"+jsforecast.forecast.simpleforecast.forecastday[0].low.celsius+"&deg");
					for (x=0;x<4;x++){
						$("#forecast"+(x+1)).html('<img src="'+getIcon(jsforecast.forecast.simpleforecast.forecastday[x].icon)+'" width="80px" height="50px">');
						...
						$("#day"+(x+1)).html("<br>"+jsforecast.forecast.simpleforecast.forecastday[x].date.weekday.substring(0,2));
					}
		...
		<div id="weather_main">
			<table id="weather"><tr><td colspan='2' id="weather_temp"></td><td colspan='2' id="weather_icon"></td></tr>
