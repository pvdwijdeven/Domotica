...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="css/OV2.css?v=3.0">
		...
		<script type="text/javascript">
			<!--
				...
				function routeSel(tempEl) {
					...
					window.location.href = "OV_iframe2.php?ID="+ID;
					
				}
				
				function showOpt() {
					document.getElementById("OVtable").style.left = "0px";
					document.getElementById("showquick").style.left = "-2000px";
					setTimeout(function(){hideOpt(); }, 300000);
				}
				
				function hideOpt() {
					document.getElementById("OVtable").style.left = "-2000px";
...
function getsymbols(text,size=16){
...
function showtable(routes,routedesc){
	...
	if (shortest==99999999){
		...
		$("#OV_header").width="526px";
		$("#OV_header1").width="526px";
		while (parseInt($("#OV_header1").css("height"))>50){
		...
		quick+="<tr class='evenrow'><td colspan="+routes[shortestID].length+"><div style='width: 526px'>vertrek: "+routedesc[shortestID][0]+" - aankomst: "+routedesc[shortestID][1]+" ("+routedesc[shortestID][2]+")</div></td></tr><tr  class='oddrow'>";
		...
		for (x=0;x<routes.length;x++){
			if(routes[x].length>0){
				...
				for (y=0;y<routes[x].length;y++){
					...
					if (shortestID==x){
						quick+="<td>"+getsymbols(routes[x][y][0],20)+"</td>";
					}
				}
		...
		$("#OV_header").css('width',"526px");
		$("#OV_header1").css('width',"526px");	
		while (parseInt($('#OV_halte').css('width'))>527){
			//console.log(parseInt($('#OV_halte').css('width')));
